const storyNodes = {
  ...
  kennel_flag: {
    ...
    choices: [{ label: "Return to main console.", next: "hub" }],
  },
  kennel_ignore: {
  ...
  lab_careful: {
    ...
    choices: [
      {
        label: "Cross-reference with crew manifest.",
        next: "hub",
        effect: (state) => {
          state.notes.push("flagged_sample");
        },
      },
    ],
  },
  lab_rush: {
    ...
    choices: [
      {
        label: "Pretend you didn't see that. Return to main console.",
        next: "hub",
      },
    ],
  },
  hub: {
    text: (state) => {
      let base =
        "The station groans as another gust hits. Lines of data crawl across the console.";
      if (state.caution >= 2) {
        base +=
  ...
  ending_isolate: {
    ...
    choices: [{ label: "Run again from the beginning.", next: "intro" }],
    ending: "good",
  },
};
